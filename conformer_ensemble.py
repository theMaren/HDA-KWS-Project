...
def build_model(input_shape, num_labels, d_model, num_heads, kernel_size, dropout_rate,subsample_stride):
    ...
    x = tf.keras.layers.Conv2D(d_model, kernel_size=(4, 4), strides=subsample_stride, activation='relu', padding='same')(inputs)
...
class ConformerHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        # Hyperparameters
        d_model = hp.Int('d_model', min_value=12, max_value=96, step=12)
        #filters = hp.Int('filters', min_value=8, max_value=96, step=8)
        ...
        model.compile(optimizer=Adam(learning_rate=1e-3, beta_1=0.9, beta_2=0.98, epsilon=1e-9),
